...
#  include "../../parameters"
  ...
  real, parameter :: fnu=omega_nu/omega_m
...
!      read(11,*) tf
      do k=1,nk
         read(11,*) tf(1,k),tf(2,k),tf(3,k),tf(4,k),tf(5,k),tf(6,k),tf(7,k),tf(8,k)
      end do
      close(11)
...
#endif
         tf(7,k)=kr**(3+ns)*tf(7,k)**2/(2*pi**2)
         tf(8,k)=kr**(3+ns)*tf(8,k)**2/(2*pi**2)
    ...
    call mpi_bcast(tf,8*nk,mpi_real,0,mpi_comm_world,ierr) 
...
subroutine veltransfer
    ...
    if (rank == 0) then
        ...
        open(11,file=vfntf)
        do k=1,nk
            read(11,*) vtf(1,k),vtf(2,k),vtf(3,k),vtf(4,k),vtf(5,k),vtf(6,k),vtf(7,k),vtf(8,k)
        end do
        close(11)
    ...
    call mpi_bcast(vtf, 8*nk, mpi_real, 0, mpi_comm_world, ierr)
...
!!$  end function Dgrow
  ...
  function Dgrow(a1,a2)
    implicit none
    real, parameter :: om=omegam
    real, parameter :: ol=omegal
    real ::  a1,a2,g1,ga1,g2,ga2
    real :: hsq,oma,ola
    real :: Dgrow
    ...
    Dgrow=( (a1*ga1)/(a2*ga2) )**(1.-3.*fnu/5.)
    return
  end function Dgrow

!!------------------------------------------------------------------!!
  !Computes a*(dD/dt)/D in simulation units 
  function vfactor(a)
    implicit none
    real :: a
    real :: H,km,lm
